...
def forecast_with_model(model, steps=12):
    ...
    forecast_band_inf  = forecast.conf_int(alpha=0.2).iloc[:, 0]
    forecast_band_sup  = forecast.conf_int(alpha=0.2).iloc[:, 1]
...
def main_prediction(data,user_selection):
    ...
    try:
        ...
        return best_model, filtered_data
...
selection_columns = sorted(['Code journal', 
...
if submit_button:
    ...
    with st.container():
        ...
        best_model, filtered_data = main_prediction(data, user_selection)
        ...
        if best_model:
            ...
            st.error("La combinaision des filtres selectionnée ne permet pas d'effectuer une prévision valable: ")
