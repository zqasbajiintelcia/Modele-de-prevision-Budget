...
with st.expander("Appliquer des filtres spécifiques"):
        ...
        for i, (col_name, values) in enumerate(options.items()):
            with cols[i % 4]:
                selected_values = st.multiselect(col_name, options=values, placeholder='Choisir les valeurs')
...
with st.expander("Show Total Budget by Rubrique"):
    st.subheader("Total Budget by Rubrique")
...
with st.expander("Show Heatmap for Budget by Month and Project"):
    st.subheader("Budget by Month and Project")
...
with st.expander("Show Pie Chart for Most Frequent Section"):
...
with st.expander("Show Stacked Area Chart for Budget by Site"):
    st.subheader("Budget by Site Over Time")
